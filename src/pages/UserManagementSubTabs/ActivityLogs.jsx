  ...
  const ActivityLogs = () => {
    ...
    const logSize = eventLogs.length;
    ...
    const handleCheckAll = () => {
      ...
      if (next) {
        const all = {};
        eventLogs.forEach((_, i) => { all[i] = true; });
        setCheckedRows(all);
      } else {
        setCheckedRows({});
      }
    ...
    const handleRowCheck = (i) => {
      setCheckedRows(prev => {
        ...
        const allChecked = logSize > 0 && eventLogs.every((_, idx) => updated[idx]);
        setCheckAll(allChecked);
        return updated;
      });
    };

    // ── Custom Checkbox (same as Reports & UserList) ──
  ...
  duration-150 ${checked ? "bg-cyan-500" : "bg-gray-800 border border-gray-600"}`}
    ...
    return (
      <div className={`grid ${width > 600 ? "grid-rows-1 grid-cols-[6fr_250px]" : "grid-cols-1"}  h-full gap-2
  [&>div]:bg-surface [&>div]:rounded-md `}>
          ...
          <div className="p-2 flex flex-row gap-3 border-b border-border shrink-0 overflow-y-auto">
            ...
            <div className="flex items-center shrink-0">
              <div className="flex flex-row items-center relative w-full">
                ...
                <input
                  className="placeholder:text-gray-500 bg-surface-2 text-sm outline-none border border-border w-full
   py-1 rounded-md pl-6 text-white focus:border-cyan-400 transition-colors duration-200"
                  type="text"
            ...
            <div className="flex items-center p-0.5 shrink-0">
              <div
                className="relative bg-surface-2 flex flex-row items-center justify-between w-full px-2 border
  border-border h-full rounded-md cursor-pointer"
            ...
            {/* Date range */}
            <div className="flex items-center shrink-0">
              <div className="text-text-muted w-full flex flex-row items-center border border-border justify-center
  py-0.5 rounded-md bg-surface-2">
                <Calendar size={20} />
            ...
            {/* Export */}
            <div className="flex items-center shrink-0">
              <button className="text-white font-bold w-full rounded-md py-0.5 flex flex-row items-center
  justify-center gap-1 bg-surface-2 border border-border cursor-pointer hover:border-cyan-400 transition-colors">
            ...
            {/* Search button */}
            <div className="flex items-center shrink-0">
              <button className="flex items-center justify-center bg-cyan-400 hover:bg-cyan-300 transition-colors
  w-full rounded-md font-bold py-0.5 cursor-pointer">
              ...
              <thead className="sticky top-0 z-10 bg-surface border-b border-border">
                <tr>
                  ...
                  {["USER", "EMAIL", "ACTION", "DESCRIPTION", "IP ADDRESS", "TIMESTAMP"].map(col => (
                    <th
                      key={col}
                      className="px-2 py-2 text-left font-semibold text-[11px] tracking-wide uppercase
  whitespace-nowrap"
                    >
                      {col}
                    </th>
  ...
  "bg-cyan-500/5" : "hover:bg-surface-2/60"}`}
                    >
                      ...
                      <td className="w-10 px-3 py-2.5 text-center">
                        <Checkbox checked={isChecked} onClick={() => handleRowCheck(idx)} />
                      </td>

                      {/* User */}
                      <td className="px-2 py-2.5 min-w-[150px]">
                        <div className="flex flex-row items-center gap-2">
                          <div className="text-base text-white bg-gradient-to-bl from-purple-500 to-pink-500 w-9 h-9
   rounded-full flex items-center justify-center font-extrabold flex-shrink-0">
                      ...
                      <td className="px-2 py-2.5 min-w-[110px]">
                        <span className={`inline-flex flex-row items-center gap-1 ${event.action.bgCol}
  ${event.action.textCol} px-2 py-0.5 rounded-full border ${event.action.border} whitespace-nowrap text-xs`}>
          ...
          <div className="flex flex-col overflow-y-auto custom-scrollbar flex-1">
            {activeUsers.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className="grid grid-cols-[1fr_1fr] gap-2 p-2 items-center border-b border-border
  hover:bg-surface-2/50 transition-colors"
                >
                  {/* Left: avatar + name */}
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold
  bg-linear-to-br from-purple-500 to-cyan-400 shrink-0 text-sm">
                      {initial}
                    </div>
